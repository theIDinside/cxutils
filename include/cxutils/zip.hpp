...
namespace cxutils
{
  ...
  public:
    ZipThree(ItA itA, ItB itB, ItC itC) noexcept : iter_a(itA.begin()), iter_b(itB.begin()), iter_c(itC.begin()),
    ...
    bool operator!=(const ZipThree &) const { return iter_a != iter_a_end && iter_b != iter_b_end && iter_c != iter_c_end; }
    ...
    auto operator*() const -> Zipped { return {*iter_a, *iter_b, *iter_c}; }
  };

  template<Iterable ItA, Iterable ItB, Iterable ItC>
  auto zip_three(ItA &&iterA, ItB &&iterB, ItC&& iterC) {
    ...
    public:
        ...
        ZipTwo(ItA itA, ItB itB) noexcept : iter_a(itA.begin()), iter_b(itB.begin()),
                                             iter_a_end(itA.end()), iter_b_end(itB.end()) {}
        const auto begin() const { return *this; }
        const auto end() const { return *this; }

        bool operator!=(const ZipTwo &) const { return iter_a != iter_a_end && iter_b != iter_b_end; }
    ...
    template<Iterable ItA, Iterable ItB>
    auto zip_two(ItA &&iterA, ItB &&iterB) {
